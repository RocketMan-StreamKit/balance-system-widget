  ...
  const schema: AddonConfigSchema = [
    ...
    },
    {
      ...
      editor: {
        ...
        description: {
          en: `When enabled, codes appear automatically on a random schedule. When disabled, use "${buttonLabel.en}" to display a code manually.`,
          ru: `Если включено — коды появляются автоматически по расписанию. Если выключено — используйте кнопку «${buttonLabel.ru}» для ручного показа.`,
          uk: `Якщо увімкнено — коди з'являються автоматично за розкладом. Якщо вимкнено — натисніть «${buttonLabel.uk}» для ручного показу.`,
        },
      },
    },
    {
